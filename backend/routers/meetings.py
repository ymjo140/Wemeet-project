...
FALLBACK_COORDINATES = {
    ...
    "안양": (37.4016, 126.9228), "수원": (37.2656, 127.0000), "강남": (37.4980, 127.0276), "역삼": (37.5006, 127.0364),
    "신논현": (37.5045, 127.0250), "삼성": (37.5088, 127.0631), "잠실": (37.5132, 127.1001), "건대입구": (37.5407, 127.0702),
    ...
    "서초": (37.4918, 127.0076), "교대": (37.4934, 127.0140), "연신내": (37.6190, 126.9210), "불광": (37.6104, 126.9298),
    "경복궁": (37.5757, 126.9735), "안국": (37.5765, 126.9854), "충무로": (37.5612, 126.9942), "약수": (37.5543, 127.0107),
    ...
    "과천": (37.4330, 126.9965), "범계": (37.3897, 126.9507), "김포공항": (37.5624, 126.8013), "여의도": (37.5215, 126.9243),
    "공덕": (37.5435, 126.9515), "광화문": (37.5710, 126.9768), "청구": (37.5602, 127.0138), "군자": (37.5571, 127.0794),
    "천호": (37.5386, 127.1236), "올림픽공원": (37.5162, 127.1309), "이태원": (37.5345, 126.9943), "한강진": (37.5396, 127.0017),
    "안암": (37.5863, 127.0292), "고려대": (37.5905, 127.0358), "석계": (37.6148, 127.0656), "망원": (37.5559, 126.9099),
    ...
    "문정": (37.4858, 127.1225), "모란": (37.4321, 127.1290), "마곡나루": (37.5667, 126.8272),
    "선정릉": (37.5102, 127.0438), "봉은사": (37.5142, 127.0602), "종합운동장": (37.5109, 127.0736),
    "판교": (37.3947, 127.1112), "분당": (37.3830, 127.1217), "일산": (37.6584, 126.7636), "인천": (37.4424, 126.6991),
...
def expand_tags_to_keywords(purpose: str, user_tags: List[str]) -> List[str]:
    ...
    if user_tags:
        for tag in user_tags:
            if tag in TAG_KEYWORD_EXPANSIONS: keywords.extend(TAG_KEYWORD_EXPANSIONS[tag][:5])
            keywords.append(tag)
        return list(dict.fromkeys(keywords))
    return PURPOSE_CONFIG.get(purpose, {}).get("keywords", ["맛집"])
...
# 🌟 [4] Core Logic (이 부분에서 유저 정보를 DB에서 불러오도록 수정됨)
...
def run_group_recommendation(req: RecommendRequest, db: Session):
        ...
        try:
            ...
            if db_user:
                participants.append({
                    ...
                    "lat": db_user.lat, "lng": db_user.lng,
                    "preferences": db_user.preferences or {} # 🌟 실제 취향 데이터 로드
                })
            else:
                # DB에 없는 경우(게스트 등) 입력값 사용
                loc = u.get('location', {})
                participants.append({
                    "id": u.get("id", 0), "name": u.get("name", "User"), 
                    "lat": float(loc.get('lat', 0)), "lng": float(loc.get('lng', 0)),
                    "preferences": {} # 빈 취향
                })
        except: continue
    ...
    for region in regions:
        try:
            ...
            if r_name == "지리적 중간" or r_name == "중간지점": r_name = TransportEngine.get_nearest_hotspot(region['lat'], region['lng'])
            ...
            # 🌟 [핵심] 실제 취향 데이터를 가진 UserProfile 생성
            algo_users = [
                agora_algo.UserProfile(
                    id=p.get('id',0), 
                    preferences=p.get('preferences', {}), # 👈 여기서 DB 데이터를 주입
                    history=[]
...
class MeetingFlowEngine:
    ...
    def plan_meeting(self, req: MeetingFlowRequest, db: Session) -> Dict[str, Any]:
        part_dicts = []
        # 🌟 [수정] DB에서 멤버 정보(취향 포함) 로드
        if req.room_id:
             try:
                 # room_id가 숫자면 int, 문자면 그대로 처리
                 rid = req.room_id
                 # 커뮤니티(채팅방) 정보 조회
                 room = db.query(models.Community).filter(models.Community.id == str(rid)).first()
                 if room and room.member_ids:
                     users = db.query(models.User).filter(models.User.id.in_(room.member_ids)).all()
                     for u in users: 
                         part_dicts.append({
                             "id": u.id, "name": u.name, "lat": u.lat, "lng": u.lng, 
                             "preferences": u.preferences or {} # 🌟 취향 반영
                         })
             except: pass

        if req.participants:
            for p in req.participants: 
                # req로 들어온 참여자도 DB 확인 시도
                db_user = db.query(models.User).filter(models.User.id == p.id).first()
                prefs = db_user.preferences if db_user else {}
                part_dicts.append({"id": p.id, "name": p.name, "lat": p.lat, "lng": p.lng, "preferences": prefs})
