...
# 🌟 [핵심 수정] 참여자가 없으면 '내 위치'를 기본값으로 사용하도록 매개변수 추가
...
def run_group_recommendation(req: RecommendRequest, db: Session):
    ...
    if len(participants) > 1:
        try:
            # 🌟 [수정] 내 위치 정보(req.current_lat/lng)를 함께 전달
            ...
            # 실패 시에도 내 위치가 아니라 참여자 평균 위치 사용
            avg_lat = sum(p['lat'] for p in participants) / len(participants)
            avg_lng = sum(p['lng'] for p in participants) / len(participants)
            regions = [{"region_name": "중간지점", "lat": avg_lat, "lng": avg_lng}]
    elif len(participants) == 1:
        p = participants[0]
        regions = [{"region_name": "내 주변", "lat": p['lat'], "lng": p['lng']}]
    else:
        # 참여자가 아무도 없을 때 => 내 위치 사용
    ...
    for region in regions:
        try:
            ...
            # 🌟 [수정] 거리 제한 적용을 위해 검색어에 지역명 필수 결합
